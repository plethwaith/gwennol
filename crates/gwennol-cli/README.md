...
carries the whole content a print run's `-v` writes, and a call its
arguments as rows, a form no print run writes, both rewrapped to the
pane's width, which drops the leading indent both forms carry — and
