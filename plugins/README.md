...
`crates/sse-guest`) and the build injects the compiled module into the
manifest's `wasmModules` slot; the JSON file here remains the plugin,
and no compiled blob is committed.
