...
`crates/sse-guest`, whose tests compile it and inject the bytes into
its manifest at test time). When milestone 4 bundles guest-backed
plugins *here*, it owes the build-time equivalent: a packaging step
